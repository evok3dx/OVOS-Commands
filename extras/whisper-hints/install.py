...
def main():
    ...
    lock_path=safe(home,'.local/state/jarvis/whisper-app-hints/update.lock')
    if args.check:
        guard=lock_path.open('rb') if lock_path.is_file() else nullcontext()
    else:
        state.mkdir(parents=True,exist_ok=True,mode=0o700)
        guard=lock_path.open('a+b')
